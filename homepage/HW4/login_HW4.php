<?php
  ...
  if (!isset($_POST['unm'])) { //not a login attempt from login.php
    if (isset($_SESSION['userid']) && $_SESSION['userid'] != "") { //This is a continuing session, let's see if it's valid
  //    header("Location: http://ecs.fullerton.edu/~cs431s17/HW4/photoshare.php");
  //    die();
    ...
    try {
      ...
      if (mysql_numrows($result) == 1) { //We have a valid user
        ...
        header("Location: http://ecs.fullerton.edu/~cs431s17/HW4/photoshare.php?loadpage=search");
        die();
...
<html>
  <head>
    <title>Image Share Log In</title>
  </head>
  <body>
    <h1>Log in to Image Share</h1>
    <hr>

    <form method=POST action="login_HW4.php">
    <b>Please enter your username and password</b>
    <p>
    <div>
      <label for="unm"><b>USERNAME: </b></label>
